...
const StepIndicator: React.FC<StepIndicatorProps> = ({ steps, currentStep }) => {
  ...
  return (
    <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3 md:gap-4 mb-4 sm:mb-6">
          ...
          <div 
            className={`w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 flex items-center justify-center rounded-full border-2 text-xs sm:text-sm font-semibold
              ...
              : stepIndex < currentStep
                ...
                : 'bg-gray-100 text-gray-500 border-gray-300'} // Future step
            `}
          ...
          {stepIndex < steps.length && (
            <div
              className={`w-5 sm:w-7 md:w-9 h-1 sm:h-1.5 md:h-2 rounded 
                ${stepIndex < currentStep ? 'bg-[#FF0000]' : 'bg-gray-200'}`}
              aria-hidden="true"
            />
          )}
        </div>
      ...
      {/* Step counter text */}
      <span className="hidden sm:inline-block ml-2 md:ml-4 text-xs sm:text-sm text-gray-500">
        Step {currentStep} of {steps.length}
      </span>
      
      {/* On mobile, show step name */}
      <span className="sm:hidden ml-2 text-xs font-medium text-gray-600">
        {steps[currentStep-1]}
      </span>
    </div>
  );
