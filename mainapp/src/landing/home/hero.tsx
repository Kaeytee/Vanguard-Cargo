		...
		<section className="relative w-full h-[600px] overflow-hidden">
			...
			{slides.map((slide, index) => {
				...
				return (
					<div
						key={slide.id}
						className={`absolute inset-0 bg-cover bg-center z-0 ${index === currentSlide || index === prevSlideIndex
							? `${getAnimationClass()}`
							: 'hidden'
							}`}
						style={{
			...
			<button
				...
				className="absolute left-6 md:left-8 top-1/2 z-20 transform -translate-y-1/2 w-8 h-8 md:w-10 md:h-10 rounded-full bg-black/30 hover:bg-black/50 text-white flex items-center justify-center transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-white/50"
			...
			<button
				onClick={nextSlide}
				className="absolute right-6 md:right-8 top-1/2 z-20 transform -translate-y-1/2 w-8 h-8 md:w-10 md:h-10 rounded-full bg-black/30 hover:bg-black/50 text-white flex items-center justify-center transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-white/50"
			...
			<div className="relative z-10 container mx-auto px-4 h-full flex flex-col justify-center text-white">
				{slides.map((slide, index) => {
					...
					return (
						<div
							key={slide.id}
							className={`absolute  left-0 right-0 ${(index === currentSlide || index === prevSlideIndex)
						...
						>
							{(index === currentSlide || index === prevSlideIndex) && (
								<>
									...
									<p className="text-xl md:text-2xl mb-8 max-w-3xl animate-fadeIn animation-delay-200">
										{slide.description}
									</p>
			...
			<div className="absolute bottom-6 left-0 right-0 flex justify-center gap-1 z-20">
				{slides.map((_, index) => (
					<button
						...
						className={`h-2 transition-all duration-300 rounded-full ${index === currentSlide
							? "w-8 bg-white"
