...
export const TTARIUS_FACILITIES: Record<string, LocationPoint> = {
  ...
  ACCRA_SORTING_CENTER: {
    latitude: 5.6108,
    longitude: -0.1821,
    address: 'Ttarius Sorting Center, Spintex Road',
    city: 'Accra',
    region: 'Greater Accra',
    country: 'Ghana',
  ...
  KUMASI_DISTRIBUTION_HUB: {
    latitude: 6.6885,
    longitude: -1.6244,
    address: 'Ttarius Distribution Hub, Adum',
    city: 'Kumasi',
    region: 'Ashanti',
    country: 'Ghana',
  ...
  TAMALE_DELIVERY_HUB: {
    latitude: 9.4034,
    longitude: -0.8424,
    address: 'Ttarius Delivery Hub, Central Market Area',
    city: 'Tamale',
    region: 'Northern',
    country: 'Ghana',
  ...
  CAPE_COAST_DELIVERY_HUB: {
    latitude: 5.1053,
    longitude: -1.2466,
    address: 'Ttarius Delivery Hub, Commercial Street',
    city: 'Cape Coast',
    region: 'Central',
    country: 'Ghana',
...
export const STATUS_LOCATION_MAPPINGS: StatusLocationMapping[] = [
  // 🔄 INTAKE PHASE LOCATIONS
  {
    ...
    location: TTARIUS_FACILITIES.ACCRA_MAIN_WAREHOUSE,
    description: 'Package request received at main warehouse',
    estimatedDuration: 24,
    isVisible: true,
    icon: 'warehouse',
    nextLocations: [TTARIUS_FACILITIES.ACCRA_SORTING_CENTER]
  },
  ...
  // 📋 GROUPING PHASE LOCATIONS
  {
    ...
    location: TTARIUS_FACILITIES.ACCRA_SORTING_CENTER,
  ...
  
  {
    ...
    icon: 'grouped',
    nextLocations: [
      TTARIUS_FACILITIES.KUMASI_DISTRIBUTION_HUB,
      TTARIUS_FACILITIES.TAMALE_DELIVERY_HUB,
      TTARIUS_FACILITIES.CAPE_COAST_DELIVERY_HUB
    ]
  },
  ...
  // 🚚 SHIPPING PHASE LOCATIONS
  {
    ...
    nextLocations: [
      TTARIUS_FACILITIES.KUMASI_HIGHWAY_JUNCTION,
      TTARIUS_FACILITIES.ACCRA_TEMA_HIGHWAY
    ]
  },
  
  {
    status: PackageStatus.IN_TRANSIT,
    location: TTARIUS_FACILITIES.KUMASI_HIGHWAY_JUNCTION, // Dynamic based on destination
  ...
  
  {
    ...
    location: TTARIUS_FACILITIES.KUMASI_DISTRIBUTION_HUB, // Dynamic based on destination
  ...
  
  {
    ...
    location: TTARIUS_FACILITIES.KUMASI_DISTRIBUTION_HUB, // Will be customer location
  ...
  // ⚠️ EXCEPTION HANDLING LOCATIONS
  {
    ...
    location: TTARIUS_FACILITIES.KUMASI_HIGHWAY_JUNCTION, // Dynamic based on last known location
...
export class LocationMappingService {
  ...
  static getLocationForStatus(
    ...
    if (!mapping) {
      ...
      return TTARIUS_FACILITIES.ACCRA_MAIN_WAREHOUSE;
    }
  ...
  static getDestinationHub(destination: string): LocationPoint {
    ...
    if (destinationLower.includes('kumasi')) {
      return TTARIUS_FACILITIES.KUMASI_DISTRIBUTION_HUB;
    } else if (destinationLower.includes('tamale')) {
      return TTARIUS_FACILITIES.TAMALE_DELIVERY_HUB;
    } else if (destinationLower.includes('cape coast')) {
      return TTARIUS_FACILITIES.CAPE_COAST_DELIVERY_HUB;
    }
  ...
  static generateRouteWaypoints(
    ...
    if (origin.city === 'Accra' && destination.city === 'Kumasi') {
      waypoints.push(TTARIUS_FACILITIES.KUMASI_HIGHWAY_JUNCTION);
    } else if (origin.city === 'Accra' && destination.city === 'Tema') {
      waypoints.push(TTARIUS_FACILITIES.ACCRA_TEMA_HIGHWAY);
    }
